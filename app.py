...
if st.sidebar.button("Login"):
    if username in USER_CREDENTIALS and password == USER_CREDENTIALS[username]:
        ...
        st.sidebar.error("Invalid credentials")("Invalid credentials")
# Block content until logged in
...
def run_update_pipeline():
    ...
    for tab in (DAILY_TAB,MULTI_TAB):
        d = load_df(tab); dc2 = find_date_column(d.columns)
        today_df = d[d[dc2]==today_str] if dc2 else pd.DataFrame()
        for row in today_df.itertuples():
            s = getattr(row,"Stake",0); stt=getattr(row,"Status","")
            change += s if stt=="Hit" else -s if stt=="Miss" else 0
    new_bal = prev+change
...
if page=="Dashboard":
        ...
        for sport in SPORTS_LIST:
            ...
            if sub.empty: st.info(f"No recs for {sport}.")
                ...
                for _,r in sub.iterrows():
                    ...
                    prob=r['Probability']*100; units=map_units(r['Probability']); stake=round(units*uv,2)
    ...
    if st.button("🔄 Generate & Save Multi-Sport Combos"):
        board=fetch_prizepicks_board(); picks=score_and_select(board)
        combos=generate_multisport_combos(picks)
        save_multi(combos); save_log("Parlay & Moonshot",combos)
        st.success(f"Combos saved to '{MULTI_TAB}' and logged")
    try:
        ...
        if tm.empty: st.info("No multi-sport combos.")
            ...
            for _,r in tm.iterrows():
                units=map_units(r['Probability']); stake=round(units*uv,2)
                st.markdown(f"- [{r['Type']}] {r['Legs']} → {r['Payout']} ({r['Probability']*100:.1f}% win) — {units}u (${stake})")
    except Exception as e:
    ...
    if st.button("Run Update Pipeline Now"): run_update_pipeline(); st.success("Pipeline run complete!")
